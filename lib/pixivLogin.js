...
var login = function(form){
  ...
  fs.exists(config.pixiv.cookieFile,function(exists){
    ...
    //模拟登陆并写入cookie文件
      ...
      console.log("模拟登录");
      //获取post_key
      request('https://accounts.pixiv.net/login', function (error, response, body) {
        if (!error && response.statusCode == 200) {
          const env = require('jsdom').env;
          console.log(body)
          env(body, function (errors, window) {
            const $ = require('jquery')(window);
            const recaptchaV3Token = $('#recaptcha-v3-token').val();
            form.recaptcha_v3_token = recaptchaV3Token;
          });
          const matches = body.match(/postKey":"([^"]*)"/);
          if(matches){
            const postKey = matches[1];
